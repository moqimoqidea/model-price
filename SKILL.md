...
Send it as a text message, never as a Markdown one: DingTalk parses a Markdown message before rendering, and that parse turns every single newline into a space, so a three-line title arrives on one line and a paragraph ending in a URL swallows the break after it. `dws chat +messages-send --identity user --open-dingtalk-id <id> --msg-type text --text "$(cat message.txt)" -y` is the channel that keeps the message byte for byte; `dws chat +dm` looks equivalent but defaults to Markdown. A DingTalk text message holds 5120 characters, so a message must fit that — which is why a page already printed under a channel is not printed again in 来源检查, and why an offer's row names its band without repeating the model's window.

Comparison and scan share one shape: the conclusion first, the detail second, the summary of what differs last. Forward that structure rather than replacing it with a summary of your own — keep the title, the one-line conclusion, the per-channel model counts, and the detail for the channels that moved. A reader who sees only the message should learn which channel moved and what it did without opening anything else or asking a follow-up, and be able to check any amount against the page it came from.
