...
**消息有字符预算。** 一条消息得先装得下才能发出去，所以同一份报告准备了三档密度，
取「装得进 `--max-chars` 的最丰档」。默认 3000 字符，离钉钉纯文本上限 5120 留足余量——
渠道只会越来越多，而一条消息得始终是一条消息。超预算的报告会换更精简的一档重排，
而不是截断：截断会让一个价格丢掉小数点，读者也无从知道丢了什么。第一步去掉峰谷时段
与差异总结，并把每条计费方案的金额并成一行；第二步把渠道的模型 id、地域与服务方式
并进标题、去掉各渠道来源地址，并给每个可能很长的列表加上条数上限。每条压过的消息
都会说明自己压过、完整明细在哪；连最精简的一档都装不下时，它会直说这一点，而不是
当作完整报告发出去。`--max-chars` 只在目标渠道的上限与钉钉不同时才需要改。
