...
价格默认只展示标准方案，仍分别列出上下文档位等条件。fast、flex、batch 等方案
...
到点仍需官方确认实际是否下线；xAI 等渠道的旧 ID 可能自动转发并改按新模型计费，
因此不能把退役一概说成旧 ID 已不可用。智谱和 MiniMax 暂无本工具可核实的公开逐模型
时间表，会明确显示该覆盖缺口并继续监控价格目录。各官方来源和适用范围见
