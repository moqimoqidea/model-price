...
显示“上次更新时间”，其值是本次运行前最近一次成功读取并写入快照的时间，
不会因选了更早的回溯基线而改变，也不再用破折号占位。

**回溯对比。** 每次成功扫描都会按渠道留下带时间戳的历史，目录未变也照常归档。
`--since yesterday` 取昨天最后一份，`--since last-month` 取上一个自然月最后一份。
ISO 日期（如 `2026-09-19`）取当天最后一份；ISO 时间戳取不晚于该时刻的最后一份，
适合在已知发布时间之前取基线。时间戳不带时区时，按本次扫描的时区解释。
某渠道没有匹配历史时会明确报 `baseline_not_found`，不会偷换成其他日期；但本次成功结果
仍会归档，供之后对比。

每个渠道保留“最近三个月内的全部基线”与“最新 1000 份基线”的并集：高频扫描不会在
三个月内被 1000 条上限截断，低频扫描也会保住更久的 1000 个时点。旧版的
`snapshots/<provider>.json` 仍能直接参与对比，并在下一次成功扫描时自动迁入历史。
...
重复一整份目录；有渠道变化、有渠道读不到、首次运行还没有基线，或指定的
回溯时间没有匹配基线时，才输出完整消息。
